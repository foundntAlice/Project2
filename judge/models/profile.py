...
class Profile(models.Model):
    ...
    #newly created 
    minus_point = models.FloatField(default=0)
    ...
    def calculate_points(self, table=_pp_table):
        ...
        if self.points != points or problems != self.problem_count or self.performance_points != pp:
            ...
            self.total_points = points - self.minus_point
            self.performance_points = points - self.minus_point
            self.save(update_fields=['points', 'total_points', 'problem_count', 'performance_points'])
        return points
    ...
    def calculate_points(self):
        ...
        # Calculate total points for AC problems
        data = (
            public_problems.filter(submission__user=self, submission__result='AC')
                        .annotate(max_points=Max('submission__points'))  # Get max points per problem
                        .values_list('max_points', flat=True)
        )
        points = sum(data) - self.minus_point  # Sum all points for AC problems

        # Calculate the number of distinct fully solved problems (optional, but retained for consistency)
        problems = len(data)

        # Update the model if necessary
        if self.points != points or self.problem_count != problems:
            self.points = points
            self.problem_count = problems
    ...
    # Indicate this method alters data
    calculate_points.alters_data = True
    '''
    ...
    def check_totp_code(self, code):
        ...
        min_timecode = max(self.last_totp_timecode + 1)
        for timecode in range(min_timecode):
    ...
    @classmethod
    def get_user_css_class(cls, display_rank, rating):
        if rating_colors:
